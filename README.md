...
- **AI는 판매를 승인하지 않음**: 챗봇 상담 이후 실제 수령까지 가려면 반드시 약사의 명시적 승인이 필요합니다. AI가 직접 자판기를 여는 경로는 없습니다.
...
| API 서버 | FastAPI |
| 상담/자판기 연동 DB | SQLite (SQLAlchemy) |
| 버전 관리 | Git + GitHub |
...
DAILY_API_KEY=       # Daily.co API 키 (화상 상담용, 없어도 나머지 기능은 정상 동작)
```
...
챗봇 화면의 "🩺 약사와 상담하기" 버튼 → **실제 화상 상담(Daily.co) 연결**(대화 내용 자동 전달) → **약사가 대화를 보고 약을 정해 승인**(처방) → QR 로그인 → 수령까지의 흐름을 구현했습니다. 사용자는 약품 코드를 몰라도 되고, AI는 이 흐름 어디에서도 최종 승인 권한이 없습니다. 자세한 설계 배경과 아직 스텁인 부분(인증, 자판기 하드웨어)은 [`CLAUDE.md`](./CLAUDE.md)의 "9. 약사 상담 + 자판기 QR 연동"을 참고하세요.
...
**주의**: 이 서브시스템은 SQLite(`data/moyak.db`)를 씁니다. Render 무료 인스턴스는 디스크가 휘발성이라 재배포/재시작마다 기록이 초기화됩니다 — 데모 중에는 문제없지만, 실제 서비스로 가려면 `DATABASE_URL`을 Postgres 등으로 바꿔야 합니다.
...
│   └── consult/           # 약사 상담 + 자판기 연동 (모델/DB/비즈니스 로직)
...
5. 🟡 자판기 재고/위치 연동 — 상담→승인→QR→수령 흐름은 프로토타입으로 구현 완료. 실제 재고 DB, MQTT 통신, 화상상담 SDK, 사용자/약사 인증은 아직
6. 사진 기반(멀티모달) 약 식별
