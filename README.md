...
챗봇 화면의 "🩺 약사와 상담하기" 버튼 → 화상상담 요청(대화 내용 자동 전달) → **약사가 대화를 보고 약을 정해 승인**(처방) → QR 로그인 → 수령까지의 흐름을 최소 기능으로 구현했습니다. 사용자는 약품 코드를 몰라도 되고, AI는 이 흐름 어디에서도 최종 승인 권한이 없습니다. 자세한 설계 배경과 아직 스텁인 부분(인증, 화상상담, 자판기 하드웨어)은 [`CLAUDE.md`](./CLAUDE.md)의 "9. 약사 상담 + 자판기 QR 연동"을 참고하세요.
