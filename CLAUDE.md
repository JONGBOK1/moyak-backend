...
**흐름**: 챗봇 상담(`/chat`) → 화상상담 요청 → **약사가 명시적으로 승인/거절** → 승인 시 "승인된 구매 건" 생성(유효시간 있음) → 사용자가 자판기 QR을 앱으로 스캔 → 대기 중인 승인 건이 있으면 수령 확정 → 자판기 개방(모의).

- `POST /consultations` — 상담 요청 생성 (챗봇 대화 요약 포함)
- `GET /consultations?status=pending` — 대기 목록 (약사 대시보드용)
- `POST /consultations/{id}/decision` — 약사 승인/거절. 승인 시에만 `ApprovedPurchase` 생성(기본 60분 유효)
...
**중요한 설계 원칙**: AI(챗봇)는 이 흐름 어디에도 "승인" 권한이 없다. 승인은 항상 `pharmacist_id`가 명시적으로 호출하는 `/consultations/{id}/decision`에서만 발생하고, 모든 승인/수령 기록이 DB에 남는다(감사 로그 역할).
